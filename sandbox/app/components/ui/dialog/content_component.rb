    ...
    class ContentComponent < UiComponent
      ...
      def close_button
        ...
        tag.button(type: "button", class: close_button_classes, data: { slot: "dialog-close-x", action: "click->ui-dialog#close" }) do
          safe_join([ render(Ui::IconComponent.new("x")), tag.span(I18n.t("ui.dialog.close", default: "Close"), class: "sr-only") ])
        end
      end

      def close_button_classes
        "absolute right-4 top-4 rounded-xs opacity-70 transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 ring-offset-background disabled:pointer-events-none"
      end
