...
module Ui
  module Popover
    # The floating panel. Hidden until the ui-popover controller opens it.
    # `side` offsets it from the trigger; `align` shifts it along the cross
    # axis (horizontal for top/bottom, vertical for left/right).
    class ContentComponent < UiComponent
      ...
      def content_attrs
        html_attrs.dup.tap do |attrs|
          attrs[:role] = attrs.fetch(:role, "dialog")
      ...
      def content_data(existing_data)
        existing_data.to_h.dup.tap do |data|
          ...
          data[:ui_popover_target] = append_token(data[:ui_popover_target], "content")
        end
      end
      ...
      def base_classes
        "absolute z-50 w-72 rounded-md border bg-popover p-4 text-popover-foreground shadow-md outline-none data-[state=open]:animate-popover-in"
      end
    end
  end
end
