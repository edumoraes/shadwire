Rails.application.routes.draw do
  ...
  # The site is bilingual and is published as static HTML, so the language has
  # to live in the URL: no server survives to the point of serving a page, and
  # nothing is left to negotiate one. English is the default and keeps the bare
  # paths (/docs); Portuguese is prefixed (/pt/docs). Constraining the optional
  # segment to the one non-default locale gives every page exactly one address
  # per language, which is also what the hreflang pair in the head claims.
