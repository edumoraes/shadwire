...
class DrawerComponentTest < ViewComponent::TestCase
      ...
      ui_drawer do
        ui_drawer_trigger { "Open" } +
          ui_drawer_content do
            ui_drawer_header do
              ui_drawer_title { "Title" } + ui_drawer_description { "Description" }
            end +
              ui_drawer_footer { ui_drawer_close { "Close" } }
          end
      end
    end
  end
  ...
  def test_trigger_opens_drawer
    render_inline(Ui::Drawer::TriggerComponent.new) { "Open" }

    assert_selector "button[aria-haspopup='dialog'][data-slot='drawer-trigger'][data-action='click->ui-drawer#open']", text: "Open"
  end
  ...
  def test_helper_composes_drawer
    ...
    assert_selector "[data-controller='ui-drawer'] button[data-slot='drawer-trigger']", text: "Open"
