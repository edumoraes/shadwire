...
module Ui
  module ContextMenu
    # A single menu action. Renders a <button> by default, or an <a> when `tag: :a`.
    class ItemComponent < UiComponent
      ...
      def item_data(existing_data)
        existing_data.to_h.dup.tap do |data|
          ...
          data[:ui_context_menu_target] = append_token(data[:ui_context_menu_target], "item")
          data[:action] = append_token(data[:action], "click->ui-context-menu#select")
